...
library(sn)             # need this for skewed normal data generation
#############################################################################
...
numbersamples = 10      # set k, the number of studies per meta-analysis
rho = .42               # population correlation = rhoTxTy
sigmarho = .20          # population standard deviation, SDrho
pop.tausq = sigmarho^2  # random-effects variance component in the population
numbermetas = 10000       # number of metas, each based on numbersamples - replications
# Set the condition of the run.
# Condition can be Normal, LeftSkew, or RightSkew for the underlying disribution of rho
#
Condition = 'RightSkew'
######################################################################
set.seed(1234)                            # set random number generator to check consistency
################
# Generate data from which to sample population values of the correlation 
# according to the condition (underlying Normal, LeftSkew, or RightSkew).
################
if (Condition == 'Normal'){
  distribution <- rnorm(10000000,rho,sigmarho)  #10 million random numbers from normal
...
if (Condition != 'Normal') {
  rhoskew = 10 #rho skew is set constant across all skewed distributions
               ...
               #omega is sd
               #alpha is skewness
  skew.parm1 <- c(.99, .61, .1936, .28) # .14, .08
  skew.parm2 <- c(.75, .61, .2587, .28) # .14, .13
  skew.parm3 <- c(.75, .61, .3479, .28) # .14, .20
  
  skew.parm4 <- c(.80, .61, .2704, .52) # .26, .08
  skew.parm5 <- c(.80, .61, .3354, .52) # .26, .13
  skew.parm6 <- c(.80, .58, .4567, .52) # .26, .20
  
  skew.parm7 <- c(.80, .61, .3726, .84) # .42, .08
  skew.parm8 <- c(.80, .61, .4359, .84) # .42, .13
  skew.parm9 <- c(.80, .49, .7241, .84) # .42, .20
  
  if (rho == .14 & sigmarho == .08){skew.parms <- skew.parm1}
  if (rho == .14 & sigmarho == .13){skew.parms <- skew.parm2}
  if (rho == .14 & sigmarho == .20){skew.parms <- skew.parm3}
  if (rho == .26 & sigmarho == .08){skew.parms <- skew.parm4}
  if (rho == .26 & sigmarho == .13){skew.parms <- skew.parm5}
  if (rho == .26 & sigmarho == .20){skew.parms <- skew.parm6}
  if (rho == .42 & sigmarho == .08){skew.parms <- skew.parm7}
  if (rho == .42 & sigmarho == .13){skew.parms <- skew.parm8}
  if (rho == .42 & sigmarho == .20){skew.parms <- skew.parm9}
  ##Initial Skewed Distribution Generation
  distribution = rsn(n = 10000000, xi=rho,  omega=sigmarho, alpha=rhoskew)
...
popCR.L <- quantile(rho.distribution, .1)
popCR.U <- quantile(rho.distribution, .9)
...
#describe(rho.distribution)
#hist(rho.distribution, xlab="rho", main = "Right Skew")
########################################################
#  DESIGN ELEMENTS
########################################################
...
#describe(bigN)
#bigNGraph <- bigN[bigN <1500]
#describe(bigNGraph)
#hist(bigNGraph, main="Gamma", xlab = "Sample Size")
...
for (j in 1:numbermetas){
  ...
  for (i in 1:numbersamples){                       # do the folowing for number of samples per meta
    ...
    rho.r <- sample(rho.distribution, size=1, replace = T)  # sample a value of rho from the parameters specified
  ...
  ########################################################################
  Unit.ri <- ri/ai                              #disattenuate r
  Un.rbar <- ri/k                               # unit weighted observed r
  Unit.Mean <- sum(Unit.ri)/k                   # unit weighted disattenuated r
  Unit.V <- sum((Unit.ri-Unit.Mean)^2)/(k-1)    # unit disattenuated variance
  Unit.SEM <- sqrt(Unit.V/k)                    # disattenuated standard error of the mean
  V.eo.un <- (1-Un.rbar^2)^2/(ni-1)             # simple observed error variance for each study
  V.ec.un <- V.eo.un/A.compound^2               # sampling error corrected for reliability
  V.un <- sum(V.ec.un)/k                        # unit average corrected error
  V.rho.un <- Unit.V-V.un                       # residual error - Random Effects Variance
  if (V.rho.un < 0) {V.rho.un <- 0}             # set REVC to zero if negative
  Un.CI95.U <-Unit.Mean + 1.96*Unit.SEM         #confidence interval
  Un.CI95.L <- Unit.Mean - 1.96*Unit.SEM
  Un.CR.80.U <-Unit.Mean + 1.28*sqrt(V.rho.un)  #Attempt at credibility interval to provide comporable values to other methods        
  Un.CR.80.L <-Unit.Mean - 1.28*sqrt(V.rho.un)
  Unit.coverage <-0
  ...
  M1 <- sum(ni*ri)/sum(ni)                       # weighted mean, uncorrected
...
# Output
Output <- data.frame(numbersamples, rho, sigmarho, Condition,
...
write.table(Output, file = "O:/CAS-PSY/output.csv", row.names=F, append=T, col.names=T, sep=",")
